...
import net.mcreator.prismaexemod.gui.RandomIngotGuiWindow;
import net.mcreator.prismaexemod.gui.RandomIngotGui;
...
public class RandomOreItem extends PrismaexemodModElements.ModElement {
	@ObjectHolder("prismaexemod:random_ore")
	public static final Item block = null;
	public RandomOreItem(PrismaexemodModElements instance) {
		super(instance, 3);
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof RandomIngotGuiWindow) {
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Random Ore");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new RandomIngotGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
	...
	private static class InventoryCapability implements ICapabilitySerializable<CompoundNBT> {
		...
		private ItemStackHandler createItemHandler() {
			return new ItemStackHandler(112) {
				@Override
